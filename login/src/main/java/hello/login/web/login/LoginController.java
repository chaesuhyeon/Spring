...
//    @PostMapping("/login")
    public String loginV3(@Valid @ModelAttribute LoginForm form , BindingResult bindingResult , HttpServletRequest request){
        ...
        //로그인 성공처리
        //세션이 있으면 았는 세션을 반환, 없으면 신규 세션을 생성
    ...
    public String loginV4(@Valid @ModelAttribute LoginForm form, BindingResult bindingResult,
                          @RequestParam(defaultValue = "/") String redirectURL, // 필터에서 보내준 httpResponse.sendRedirect("login?redirectURL=" + requestURI);값
...
//    @PostMapping("/logout")
    ...
    public String logoutV3(HttpServletRequest request){
        HttpSession session = request.getSession(false); //세션을 새로 생성하면 안되므로 false해서 세션이 없으면 null을 반환하도록

        if(session != null){ //session이 null이 아니면
            session.invalidate(); //정보 없애기
        }
